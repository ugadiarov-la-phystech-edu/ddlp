...
def train_dlp(config_path='./configs/shapes.json'):
    ...
    for epoch in range(pretrained_epoch, num_epochs):
        ...
        if epoch % eval_epoch_freq == 0 or epoch == num_epochs - 1:
            ...
            if math.isinf(best_valid_loss) is None or best_valid_loss > valid_loss:
            ...
            if eval_im_metrics and epoch > 0:
                ...
                print(log_str)
                log_line(log_dir, log_str)
                if (not torch.isinf(torch.tensor(val_lpips))) and (math.isinf(best_val_lpips) or best_val_lpips > val_lpips):
